...
class Prescription_model extends CI_Model
{
    ...
    public function getPrescriptionByEMG($id, $visitid = '')
    {
        if (!empty($visitid)) {
            $this->db->where("prescription.visit_id", $visitid);
        } else {
        ...
        $query = $this->db->select('prescription.*,medicine_category.medicine_category,staff.id as staff_id')->join("emg_patients_detail", "prescription.opd_id = emg_patients_detail.id")->join("medicine_category", "prescription.medicine_category_id = medicine_category.id")->join("patients", "patients.id = emg_patients_detail.patient_id")->join("staff", "staff.id = emg_patients_detail.cons_doctor")->where("prescription.opd_id", $id)->get("prescription");
    ...
    public function getmanualEmg($opdid)
    {
        $query  = $this->db->select("emg_patients_detail.*,patients.id as patientid ,patients.patient_name,patients.patient_cnic,patients.email,patients.mobileno,patients.patient_unique_id,patients.age,patients.month,patients.gender,patients.address,staff.name,staff.surname,staff.local_address")->join("patients", "patients.id = emg_patients_detail.patient_id")->join("staff", "staff.id = emg_patients_detail.cons_doctor")->where("emg_patients_detail.id", $opdid)->get("emg_patients_detail");
        $result = $query->row_array();
        return $result;
    }
