...
class Patient_model extends CI_Model
{
    ...
    public function searchipd_datatable()
    {
        ...
        $this->db->select("patients.*,'0' charges,'0' payment,'0' amountdue,bed.name as bed_name,bed_group.name as bedgroup_name, floor.name as floor_name,ipd_details.date,ipd_details.id as ipdid,ipd_details.credit_limit as ipdcredit_limit,ipd_details.case_type,ipd_details.ipd_no,staff.name,staff.surname,kpo.name as kpo_name
              ")->from('patients');
        $this->db->join('ipd_details', 'patients.id = ipd_details.patient_id', "inner");
        $this->db->join('staff', 'staff.id = ipd_details.cons_doctor', "inner");
        $this->db->join('bed', 'ipd_details.bed = bed.id', "left");
        $this->db->join('bed_group', 'ipd_details.bed_group_id = bed_group.id', "left");
        $this->db->join('floor', 'floor.id = bed_group.floor', "left");
        $this->db->join('staff kpo', 'kpo.id = ipd_details.generated_by', "left");
        $this->db->where('patients.is_active', 'yes');
        $this->db->where('ipd_details.discharged', 'no');
        if (!isset($_POST['order'])) {
            $this->db->order_by('ipd_details.id', "desc");
    ...
    public function getEarning($field, $module, $search_field = '', $search_value = '', $search = '')
    {
        ...
        $query  = $this->db->select('count(' . $field . ') as count')->get($module);
        $result = $query->row_array();
    ...
    public function getorganizationCharge($organisation)
    {
        $result      = array();
        if (!empty($organisation)) {
            $second_query = $this->db->select("org_charge")
    ...
    public function search_datatable_emg()
    {
        ...
        $this->db->select('emg_patients_detail.*,patients.id as pid,patients.patient_name,patients.patient_cnic,patients.patient_unique_id,patients.guardian_name,patients.gender,patients.mobileno,patients.is_ipd,staff.name,staff.surname')->from('emg_patients_detail');
